...
- 以 `NDataTable` 顯示樓層清單（依 `displayOrder`），欄位：名稱、座位數、底圖狀態（`NTag` 色彩區分 `Ready` / `Failed` / 無底圖）。
...
- 上傳為**同步**轉檔，依 `POST /api/floors/{floorId}/map` 回應顯示狀態（上傳期間以 `NSpin` + 「轉檔中...」覆蓋）：
  - 成功（200，`Ready`）：`NAlert`（`type="success"`）+ 以 Leaflet `L.geoJSON` 載入底圖顯示**預覽**，管理者確認方向與比例正確（規格步驟 4）。
  - 失敗（400，`Failed`）：`NAlert`（`type="error"`）顯示 `errorMessage`（規格：解析失敗顯示錯誤並擋上傳）。
- 提供「重新上傳」（`NButton`）覆蓋舊底圖、「移除底圖」（`NButton` `type="error"`）`DELETE`。

### 步驟 4：座位編輯（地圖編輯模式）

新增 `src/components/admin/SeatEditorMap.vue`（重用模組 10 的地圖基礎）：
- 載入該樓層 GeoJSON 底圖與既有座位。
